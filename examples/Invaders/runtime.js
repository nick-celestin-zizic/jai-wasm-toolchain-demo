...
const create_file_and_folders = async (full_path) => {
    ...
    for (let it_index = 0; it_index < parts.length-1; it_index++) {
        ...
        if (it === ".") continue;
        else if (it === "..") throw new Error("TODO: backtrack");
        
        try { folder = await folder.getDirectoryHandle(it); }
        catch (e) { folder = await folder.getDirectoryHandle(it, { create: true }); }
    }
...
const find_file = async (full_path) => {
    ...
    for (let it_index = 0; it_index < parts.length-1; it_index++) {
        ...
        folder = await folder.getDirectoryHandle(it);;
    }
        ...
        try {
            ...
            try {
                ...
                if (for_writing === 1 && keep_existing_content === 0) {
                    ...
                    console.log(e);
                    throw new Error(`TODO: file ${fetch_name} found. propogate the error...`);
                    wasm_resume(1);
                    return;
                }
            }
...
exported_js_functions.webaudio_load_audio_data = (path_pointer, in_sound_data, out_sound_data) => {
    const view = new DataView(allocated.buffer);
    switch (wasm_pause()) {
    case 0: (async () => {
        const path = js_string_from_jai_string_pointer(path_pointer);
        try {
            const in_sound_data_count = Number(view.getBigInt64(Number(in_sound_data) + 0, true));
            const in_sound_data_data  = Number(view.getBigInt64(Number(in_sound_data) + 8, true));
            const buffer = new ArrayBuffer(in_sound_data_count);
            new Uint8Array(buffer).set(new Uint8Array(allocated.buffer, in_sound_data_data, in_sound_data_count));
            const audio  = await audio_context.decodeAudioData(buffer);
            sounds[path] = audio;
            wasm_resume(1);
        } catch (e) {
            // TODO: maybe we should send these to context.logger instead of doing this?
            // It would be more annoying since we cannot call it while paused
            if (e.name === "EncodingError") {
                console.error(`This browser could not decode the audio data from ${path}. WebAudio format support varies widely by browser and is largely undocumented, so maybe another browser can decode this file or maybe not! Here is the original, potentially useless, error generated by WebAudio: ${e.message}`);
            } else {
    ...
    case 1: {
        ...
        dst.set(src); // copy the path string over
...
window.onload = async () => {
    ...
    for (let it_index = 0; it_index < registrations.length; it_index++) {
        ...
        if (it.scope === document.location.href) {
            if (registration === undefined) {
                // console.log("Found worker for this page");
                registration = it;
            } else {
                console.warn("We found multiple service workers on this page, this is not supported yet. We will unregister it for now...", it);
                await it.unregister();
            }
        }
    }
    ...
    if (registration === undefined) {
        // console.log("Could not find service worker for this page, registering new one...");
        registration = await navigator.serviceWorker.register("./worker.js");
    }
    
    // force the worker to update so that we get an install or activate event no matter what
