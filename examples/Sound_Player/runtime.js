...
const sounds  = {}; // JS.string -> object URL
const streams = {}; // *Sound_Stream -> JS.Audio

exported_js_functions.webaudio_load_audio_file = (_path, optional, out_sound_data) => {
    const path = js_string_from_jai_string_pointer(_path);
    
    if (setjmp_and_suspend(yield_jmp_buf) === 0) {
        const wait_for_audio = async (url) => {
            const response = await fetch(url);
            const data     = await response.blob();
            sounds[path]   = URL.createObjectURL(data);
        };
        // wait_for_audio(path).then(() => { wasm_resume(1); });
        wait_for_audio(path).then(() => { longjmp(yield_jmp_buf, 1); });
    } else {
        const view     = new DataView(allocated.buffer);
        const count    = view.getBigInt64(Number(_path) + 0, true); // _path.count
        const old_data = view.getBigInt64(Number(_path) + 8, true); // _path.data
        ...
        view.setBigInt64(base + 0, count, true);    // result.path.count = _path.count
...
exported_js_functions.webaudio_load_audio_file2 = (_path, optional, out_sound_data) => {
    const path = js_string_from_jai_string_pointer(_path);
    ...
    if (setjmp_and_suspend(yield_jmp_buf) === 0) {
        ...
        const base = Number(out_sound_data);
        view.setBigInt64(base + 0, count, true);    // result.path.count = _path.count
        view.setBigInt64(base + 0, new_data, true); // result.path.data  = new_data; 
        view.setInt8(base + 16, 1, true);           // result.loaded = true;
    ...
    // console.log(`webaudio_load_audio_file(${path}, ${optional}) -> 0x${out_sound_data}`);
};
...
exported_js_functions.webaudio_sound_player_shutdown = () => {
    for (const [sound_stream, audio] of Object.entries(streams)) {
        // TODO: free idk prob not lol
        audio.pause();
    }
};

// @TODO: It would be nice to generate this from the metaprogram since we already need one.
// Something like a @Js note for type decls that generates offsets for structs and values for enums.
// Maybe even something like getters/setters for structs if we are feeling really crazy.
// -nzizic, 1 May 2025
const Sound_Category_GENERAL_SFX = 0;
const Sound_Category_AMBIENCES   = 1;
const Sound_Category_FOOTSTEPS   = 2;
const Sound_Category_UI          = 3;
const Sound_Category_MUSIC       = 4;
const Sound_Category_to_string   = (x) => {
    switch (x) {
    case Sound_Category_GENERAL_SFX: return "GENERAL_SFX";
    case Sound_Category_AMBIENCES: return "AMBIENCES";
    case Sound_Category_FOOTSTEPS: return "FOOTSTEPS";
    case Sound_Category_UI: return "UI";
    case Sound_Category_MUSIC: return "MUSIC";
    }
    throw new Error(`Sound_Category_to_string: corrupt value ${x}`);
}
...
// const streams = {}; // *Sound_Stream -> 
exported_js_functions.webaudio_set_repeating = (_name, repeating) => {
    console.log("webaudio_set_repeating");
    // const name = js_string_from_jai_string_pointer(_name);
    // sounds[name].loop = repeating;
};

// exported_js_functions.web

// exported_js_functions.webaudio_get_cached_stream = (_name) => {
//     // const name   = js_string_from_jai_string_pointer(_name);
//     // const stream = streams[name];
//     // console.log("got stream ", stream);
//     // return stream === undefined ? 0n : stream;
//     return 0n;
// };

// exported_js_functions.webaudio_cache_stream = (_name, stream) => {
//     // const name = js_string_from_jai_string_pointer(_name);
//     // if (streams[name] !== undefined) throw new Error("TODO: report error? free the old stream? idk man");
//     // streams[name] = stream;
//     // console.log(streams);
// };
